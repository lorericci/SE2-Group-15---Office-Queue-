...
const Display = () => {
  const [ticketInfo, setTicketInfo] = useState<{ [key: number]: number }>([12, 22, 23, 25]);
  const [lastCounter, setLastCounter] = useState(0);
  const [numCounters, setNumCounters] = useState(0);
  const [services, setServices] = useState<string[]>(['Financial', 'Atm', 'gianfranco', 'aaaaaaaaaa', 'audio']);
  const [queueLengths, setQueueLengths] = useState<number[]>([3, 5, 2]);
      ...
      <div className="services-container">
        {services.map((service, index) => (
          <div key={index} className="service-box">
            <h2>{service}</h2>
            <h3>Waiting: {queueLengths[index]}</h3>
          </div>
        ))}
      </div>
